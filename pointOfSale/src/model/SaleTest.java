...
class SaleTest {
	...
	// Extensive test that generates a long array of random products, then increments that product a random amount of times, meanwhile
	// the total cost is kept track of. The test finally sees if the calculateRunning total matches with the expected total.
	@Test
	void canCalculateCorrectTotalAmount() {
		...
		for(int i = 0; i < amountOfProducts; i++){
			...
			products[i] = new ProductDTO(i, Math.random() * reasonableMaxPriceForAProduct, 12, "green " + String.valueOf(Math.random()));
		}
		...
		assertEquals(totalAmount, sale.getRunningTotal(), epsilon,"Could not count: Got: " + sale.getRunningTotal() + ", but expected: " + totalAmount );
